...
bool CMemBuffer::AddFile(LPCTSTR szFileName, DWORD dwFlags /* = 0 */)
{
	assert(szFileName);
	CMemBuffer mbData;
...
BOOL CMemBuffer::WriteToFile(std::wstring& sFileName, DWORD dwFlags /*= MEMBUFFER_FLAG_WRITEFILE_OVERWRITE*/)
{
	return WriteToFile(sFileName.c_str(), dwFlags);
}

BOOL CMemBuffer::WriteToFile(std::wstringstream& ssFileName, DWORD dwFlags /*= MEMBUFFER_FLAG_WRITEFILE_OVERWRITE*/)
{
	return WriteToFile(ssFileName.str().c_str(), dwFlags);
}

BOOL CMemBuffer::WriteToFile(LPCTSTR szFileName, DWORD dwFlags /*= MEMBUFFER_FLAG_WRITEFILE_OVERWRITE*/)
{
	BOOL bReturn = TRUE;
	FILE *fp = NULL;
...
{
	LPCTSTR szID = MAKEINTRESOURCE(ID);
	HRSRC myResource = ::FindResource(NULL, szID, MAKEINTRESOURCE(RT_RCDATA) );
	DWORD dwError = 0;

...
BOOL ButtonMine::SetImage(LPCTSTR pszFile,LPCSTR pszToolTip,COLORREF clrBkgnd,BOOL bAutoSize,BOOL bNormalDisable)
{
...
BOOL ButtonMine::SetImage(LPCTSTR pszFile,COLORREF clrBkgnd,BOOL bAutoSize,BOOL bNormalDisable)
{
	USES_CONVERSION;
	IPicture*pPicture=0;
	...
	//return hr
	BOOL b = ::OleLoadPicturePath(T2OLE(sPath.GetBuffer()), 0, 0, clrBkgnd, IID_IPicture, (LPVOID*)&pPicture) ? 0 : 1;
...
void ButtonMine::P_Draw3dRect(HDC dc, const CRect&rc, CPen&penTL, CPen&penBR)
{
#if 0
	::MoveToEx(dc, rc.left, rc.bottom, NULL);
	::SelectObject(dc, &penTL);
	::LineTo(dc, rc.left, rc.top);
	::LineTo(dc, rc.right, rc.top);
	::SelectObject(dc, &penBR);
	::LineTo(dc, rc.right, rc.bottom);
	::LineTo(dc, rc.left, rc.bottom);
#endif
}

void ButtonMine::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	if (inf.bInit == 0)
	{
		return;
	...
	HGDIOBJ hPenOld = ::SelectObject(lpDrawItemStruct->hDC, &penB);
	//CPen*pPenOld = 
	//CPen*pPenOld = dc.SelectObject(&penB);

	if (inf.bBtnDn == 0)
	{
		P_Draw3dRect(lpDrawItemStruct->hDC, rc, penW, penB);
		if (inf.bWeaker == 0)
		{
			rc.DeflateRect(1, 1);
			P_Draw3dRect(lpDrawItemStruct->hDC, rc, penW, penB);
	...
	{
		P_Draw3dRect(lpDrawItemStruct->hDC, rc, penB, penW);
		if (inf.bWeaker == 0)
		{
			rc.DeflateRect(1, 1);
			P_Draw3dRect(lpDrawItemStruct->hDC, rc, penB, penW);
	...
	if (inf.bShowFocus && (GetFocus() == this))
	{
		...
		P_Draw3dRect(lpDrawItemStruct->hDC, rc, pen, pen);
	}
